...
#filename = 'v4Q_TABLE_4_QTvQT_20kB.pickle' # Not very good. sub 50+% wr
#filename = 'v2Q_TABLE_4_QTvRA_10k.pickle'
#filename = 'v2Q_TABLE_4_QTvHE_35k.pickle'

#filename = 'v3Q_TABLE_4_QTvQT_20k.pickle'      # bETTER than that guy ^
...
#filename = 'v3Q_TABLE_4_QTvRA_25kA.pickle'  # 60+% win rate vs RANDOM
#filename = 'v3Q_TABLE_4_QTvRA_20kA.pickle'      # Not very good. sub 50+% wr
#filename = 'v3Q_TABLE_4_QTvRA_20kB.pickle'
#filename = 'v3Q_TABLE_4_QTvRA_20kC.pickle'     # ~60+% wr

#filename = 'v3Q_TABLE_4_QTvHE_30kC.pickle'      # 60% win rate. kinda.
#filename = 'v3Q_TABLE_4_QTvHE_20kC.pickle'      # NOW its the golden god!? WTF==?? 100%
#filename = 'v3Q_TABLE_4_QTvHE_30kB.pickle'
#filename = 'the_golden_god_brother_20kC.pickle'
        ...
        while True:
            ...
            if trigger_test(new_state):
                ...
                # Else doing a random choice --> exploration
                    ...
                    index = random.choice(possiblePos(state))
                    LOG_ACTION = table_index_to_actioncombo(index)
                    action_phase2 = LOG_ACTION[1]

                print("After trigger. Time to lay his trap")
                print_board(state)
                # ***************** PHASE 2.5 COMPLETE  **************************AI
                next_state = move_phase2(state, action_phase2)
                # ****************************************************************
                # Update again before kicking it to the next player
                state = next_state
...
# -----------------------------------------|
            ...
            if str(state[:2]) not in qtable:
                # Add the Board to the dict w/ value = empty 1x72 array
                print("NEW STATE ADDED for random bot.")
            ...
            if str(new_state[:2]) not in qtable:
                # Add the Board to the dict w/ value = empty 1x72 array
                qtable[str(new_state[:2])] = np.zeros(xsize)
                print("NEW STATE ADDED after random bot.")
            ...
            if trigger_test(new_state):
                ...
                print("\n \n TIME TO LAY A TRAP! ;-)")
                state = empty_trapboard(state)
                index = random.choice(possiblePos(state))
                LOG_ACTION = table_index_to_actioncombo(index)
                action_phase2 = LOG_ACTION[1]
                print("action phase2.5:" + str(action_phase2))
                # ************** PHASE 2.5 COMPLETE ******************************HU
                next_board = move_phase2(state, action_phase2)
                # ****************************************************************
                # (^^)/ *Update* again before kicking it to the next player
                state = next_board
                state = swapBoard_me_and_her(state)
...
QT_WINS, RA_WINS, DRAWS = playgame(NUM_GAMES)

print("Total Ties: ", DRAWS)
print("Total QT-Player wins: ", QT_WINS)
print("Total Random-Player wins: ", RA_WINS)
